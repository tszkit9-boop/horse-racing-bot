...
DEFAULT_CONFIG = {
    ...
    "invite_rewards": {
        ...
        "level2": 2   # 上線(A 邀請 BB 邀請 C → A 得 2 次)
        "level3": 1   # 上上線：+1 次
    }
    # ===== 🎯 彩池設定 =====
    "pool_config": {
...
def _repair_racecard(df):
    ...
    # 中文格式欄位順序：馬號馬名檔位負磅騎師練馬師場次比賽日期賠率
    cn_cols = ['horse_id' 'horse_name' 'draw' 'weight' 'jockey'
               'trainer' 'race_no' 'race_date' 'win_odds']

    # 英文格式欄位順序：race_daterace_nohorse_nohorse_namedrawweightjockeytrainerwin_odds
...
def _build_features(race_df history_df):
    """為排位表每匹馬計算特徵(加入馬名對照修復歷史數據對唔上嘅問題)"""
    import numpy as np
    import os
...
def run_prediction(date_str race_no):
    ...
    if date_str not in available_dates:
        st.warning(f"⚠️ {date_str} 冇數據自動改用 {available_dates[-1]}")
    ...
    if race_no not in avail_races:
        st.warning(f"⚠️ {date_str} 冇第 {race_no} 場改用第 {avail_races[0]} 場")
    ...
    st.success(f"✅ 成功載入 {date_str} 第 {race_no} 場共 {len(filtered)} 匹馬")
...
def _get_pos_series(df):
    """自动选择名次欄位：优先 Pla如果冇就用 finish_position"""
    # 試 Pla(用位置索引避免隱藏字元)
    pla_idx = None
...
def admin_horse_ranking():
    ...
    if name_col is None or pos_col is None:
        ...
        st.warning("⚠️ 賽果檔案缺少「馬名」或「名次」欄位請檢查 CSV 格式。")
        return
...
def show_lottery_interface(username):
    ...
    if lottery_chances <= 0:
        st.warning("⚠️ 你冇抽獎次數啦！請聽日再嚟或者聯絡管理員增加。")
        return
    ...
    if st.session_state.lottery_result is not None:
        ...
        <div style="background: linear-gradient(135deg #f9d423 #ff4e50);
                    ...
                    color: white; box-shadow: 0 8px 25px rgba(25578800.4);
...
def admin_user_management():
    ...
    if not users:
        st.error("❌ 讀取用戶失敗請檢查 Supabase 連線")
        return
...
def admin_reward_management():
    ...
    # ============================================================
    with tab1:
        st.caption("審核用戶打賞批准後會自動加 VIP 天數。")
    ...
    # ============================================================
    with tab2:
        st.caption("喺呢度新增、修改或刪除打賞選項前台會即時同步。")
...
def admin_auto_maintenance():
    ...
    if st.button("🚀 執行維護" type="primary" use_container_width=True key="run_maint"):
        ...
        if exp:
            save_users(users)
        st.success(f"✅ 維護完成處理 {len(exp)} 位過期用戶")

def admin_automation():
    st.subheader("🤖 自動化工具")
    st.markdown("### ⚙️ 自動化排程")
    st.info("以下自動化任務由 GitHub Actions 定時執行唔需要人手操作。")
    ...
    st.markdown("---")
    st.caption("💡 提示：如果想手動觸發可以喺 GitHub 倉庫嘅 Actions 頁面撳 Run workflow。")
...
def admin_pool_config():
    st.subheader("🎯 彩池設定")
    st.caption("可以獨立開關每個彩池同設定最低會員級別。")
...
def display_race_calendar():
    try:
        ...
        st.markdown(f"""
        <div style="background:{bg};padding:20px 24px;border-radius:16px;color:white;box-shadow:0 6px 20px rgba(1021262340.35);margin-bottom:16px;">
...
def main():
    ...
    with c4:
            ...
            with st.popover("👤 個人中心" use_container_width=True):
                ...
                with st.expander("👥 我的推薦記錄" expanded=False):
                    ...
                    st.caption(f"已成功邀請 **{invite_count}** 位朋友共獲得 **{invite_rewards}** 次額外預測")
    ...
    if st.session_state.get('role') == 'super_admin':
        with st.expander("🛠️ 管理員工具：一鍵預測所有場次"):
            st.caption("⚠️ 只限管理員使用會自動預測指定日期嘅所有場次。")
            ...
            if run_batch:
                ...
                try:
                    ...
                    if not day_races:
                        ...
                        status.text("✅ 完成！")
                        st.success(f"✅ 成功 {success_count} 場失敗 {fail_count} 場")
                ...
                if len(race_list) >= 2:
                    ...
                    if double_start is not None:
                        ...
                        if len(d_races) >= 2:
                            ...
                            st.warning(f"⚠️ 由第 {double_start} 場開始唔夠 2 場數據")
                else:
                    st.warning("⚠️ 唔夠 2 場賽事冇孖寶")
                ...
                if len(race_list) >= 3:
                    ...
                    if treble_start is not None:
                        ...
                        if len(t_races) >= 3:
                            ...
                            st.warning(f"⚠️ 由第 {treble_start} 場開始唔夠 3 場數據")
                else:
                    st.warning("⚠️ 唔夠 3 場賽事冇三寶")
                ...
                if len(race_list) >= 6:
                    ...
                    if six_up_start is not None:
                        ...
                        if len(six_up_races) >= 6:
                            ...
                            st.warning(f"⚠️ 由第 {six_up_start} 場開始唔夠 6 場數據(只有 {len(six_up_races)} 場)")
                else:
                    st.warning("⚠️ 唔夠 6 場賽事冇六環彩")
    ...
    st.divider()
    st.warning("⚠️ 免責聲明：本系統預測僅供參考不構成投注建議。賽馬活動涉及風險用戶應量力而為。用戶必須年滿18歲。")
