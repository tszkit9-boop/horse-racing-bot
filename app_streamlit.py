...
# ---------- 頁面設定 ----------
st.set_page_config(page_title="SHTSN 賽馬AI預測", page_icon="horse", layout="wide")
...
def handle_refund(original_idx, record):
    ...
    st.info(f"已為 {record.get('username')} 辦理退款，用戶已降級")
    return True
...
def payment_review_page():
    ...
    for idx, record in enumerate(filtered):
        ...
        with st.container():
            ...
            with cols[0]:
                st.write(f"用戶: {record.get('username', '未知')}")
            ...
            with cols[3]:
                ...
                if record.get("review_notes"):
                    st.caption(f"備註：{record['review_notes']}")
            with cols[4]:
                if status == "pending":
                    ...
                    if st.button("退款", key=f"ref_{original_idx}"):
...
def compare_page():
    ...
    data = {"場次":[1,2,3,4,5], "預測頭馬":["美麗傳承","金鎗六十","時時精綵","將王","浪漫勇士"], "實際頭馬":["美麗傳承","金鎗六十","時時精綵","將王","浪漫勇士"], "命中":["是","是","是","否","是"]}
    st.dataframe(pd.DataFrame(data), use_container_width=True)
    st.metric("命中率", "80%")
...
def dashboard_page():
    ...
    with col2:
        remaining = get_remaining()
        st.metric("剩餘預測", "無限" if remaining == float('inf') else remaining)
    with col3:
