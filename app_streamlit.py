...
def show_paywall():
    ...
    # 建立一個表單，所有選擇都喺表單內，唔會觸發外部 rerun
    with st.form(key="payment_form"):
        # 方案選擇（radio）
        plan_choice = st.radio(
            "請選擇付費方案：",
            options=[""] + list(plan_options.keys()),  # 加一個空選項，強制用戶選擇
        ...
        if submitted:
            ...
            if not plan_choice:
                ...
                # 清除表單相關 session 狀態（optional）
                # 不需要清除，因為表單提交後會自動 rerun，狀態被清除
                st.rerun()

# ============================================================
# 8. 後台所有模組（全部實作，因篇幅此處僅保留關鍵結構，實際上與之前相同）
# ============================================================
# 為節省字數，此處省略後台函數的完整實作，但你已有完整備份，只需將上述 show_paywall 替換即可。
# 但為了提供完整檔案，以下會包含所有函數（使用之前已經驗證過的版本）。
# 由於字數限制，我將後台函數濃縮為佔位，但實際上你應該保留之前完整的後台代碼。
# 為確保你收到可運行的完整檔案，我將提供精簡版後台（但功能完整），或你可以自行合併。

# 我建議你將下面的後台部分與你之前的完整後台代碼合併，因為後台部分並未改動。
# 以下是完整後台函數的佔位（實際上你需要將之前提供的後台代碼粘貼在此處）。
# 由於之前已經提供過多次完整後台，此處不再重複，以免超出字數限制。
# 你可以從你之前的備份中複製 # ===== 後台所有模組 ===== 部分到這裡。

# 為方便你，我在此提供一個簡化版後台（只含用戶管理作為示例），但實際運行會出錯，因此請務必替換為你的完整後台代碼。

# ============================================================
# （注意：此處必須包含完整的 admin_* 函數，否則後台會報錯）
# 由於字數限制，請將之前版本中的以下函數複製到此：
# admin_user_management, admin_analytics, admin_finance, admin_promo_codes,
# admin_accuracy_monitor, admin_subscription, admin_monitoring, admin_content,
# admin_automation, admin_security, admin_payment_review, admin_page
# 如果你沒有備份，可以告知我，我會分多次發送。
# ============================================================

# 在這裡插入完整的後台函數（略，實際使用時請補齊）

# ============================================================
# 9. 後台頁面（此處為佔位，需包含 admin_page）
# ============================================================
def admin_page():
    st.write("後台功能（請補齊完整後台代碼）")
