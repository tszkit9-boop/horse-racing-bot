...
DEFAULT_CONFIG={
    ...
    "module_user_management":True,"module_analytics":True,
    "module_finance":True,"module_monitoring":True,
    "module_content":True,"module_automation":True,
    "module_security":True,"module_promo":True,
    "enable_daily_free_tip":True,"enable_invite_reward":True,
    "invite_reward_inviter":1,"invite_reward_invitee":1,
}
...
st.set_page_config(page_title="🏇 賽馬預測系統",page_icon="🐎",layout="wide",initial_sidebar_state="expanded",menu_items={'Get Help':None,'Report a bug':None,'About':None})
FINANCE_FILE='finance.json';LOG_FILE='admin_log.json';AUTOMATION_FILE='automation.json';CONTENT_FILE='content.json';PROMO_FILE='promo_codes.json';ACCURACY_FILE='accuracy.json';PAYMENT_PROOFS_FILE='payment_proofs.json';PAYMENT_PROOFS_DIR='payment_proofs'
if not os.path.exists(PAYMENT_PROOFS_DIR): os.makedirs(PAYMENT_PROOFS_DIR)
def load_json(file):
    ...
    if not users or "admin" not in users:
        ...
        for uid,u in users.items():
            ...
            if 'predictions_limit' not in u:
                if u.get('group') in ['super_admin','VIP','paid']: u['predictions_limit']=-1
                else: u['predictions_limit']=CONFIG["free_limit"]
        save_users(users)
    return users
...
def load_payment_proofs():
    ...
    if not proofs:
        ...
        save_payment_proofs(proofs)
    return proofs
def save_payment_proofs(proofs): return save_json(PAYMENT_PROOFS_FILE,proofs)
...
def authenticate(username,password):
    users=load_users()
    if username in users and users[username].get('password')==password: return True
    return False
def generate_promo_code(): return ''.join(random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',k=8))
def generate_verification_code(): return ''.join(random.choices('0123456789',k=6))
...
def safe_parse_dates(df):
    ...
    dates=df[date_col].copy().astype(str).str.strip()
    parsed=pd.to_datetime(dates,errors='coerce')
    if parsed.notna().sum()==0: return None,None
    df['race_date']=parsed
    return df,date_col
...
def compute_stats(race_df,history_df,race_date):
    ...
    for col in ['jockey','trainer','horse_id']:
        if col not in race_df.columns: race_df[col]=0
    ...
    for col in ['course_win_rate','course_avg_rank','weight_change','jockey_trainer_win_rate',
                ...
                'total_injuries','injury_severity']:
        if col not in race_df.columns: race_df[col]=0
        else: race_df[col]=race_df[col].fillna(0)
    return race_df
@st.cache_data
...
def run_prediction(date_str,race_no):
    ...
    if xgb_model is None: return None,None
    try:
    ...
    if df is None:
        st.error("無法解析日期"); return None,None
    df=df.dropna(subset=['race_date'])
    if df.empty:
        st.error("無有效日期"); return None,None
    if 'race_no' not in df.columns:
        st.error("找不到場次欄位"); return None,None
    ...
    if df.empty:
        st.error("無有效場次"); return None,None
    ...
    if 'race_date' not in history.columns:
        if '比賽日期' in history.columns:
            ...
            st.error("歷史數據缺少日期欄位"); return None,None
    ...
    if finish_col is None:
        st.error("歷史數據缺少名次欄位"); return None,None
    ...
    for f in FEATURES_EN:
        if f not in race_sel.columns: race_sel[f]=0
        else: race_sel[f]=race_sel[f].fillna(0)
...
def record_prediction(username,date_str,race_no,horse_name,predicted_prob=None):
    ...
    if username in users:
        if 'history' not in users[username]: users[username]['history']=[]
...
def get_user_stats(username):
    ...
    total=len(history); free_used=user.get('free_usage',0)
    return {'total_predictions':total,'free_used':free_used,
            'is_paid':user.get('is_paid',False),'group':user.get('group','free'),
...
def show_user_dashboard(username):
    ...
    if group=='super_admin': level="👑 超級管理員"
    elif group=='VIP': level="👑 VIP"
    elif is_paid: level="💎 付費用戶"
    else: level="🆓 免費用戶"
    st.markdown("---")
    ...
    st.markdown("---")
    if plan: st.caption(f"📌 當前方案：{get_plan_name(plan)}")
...
def show_prediction_history(username):
    if not username:
        st.info("請先登入以查看歷史記錄"); return
    users=load_users()
    if username not in users:
        st.info("未有歷史記錄"); return
    history=users[username].get('history',[])
    if not history:
        st.info("你仲未有任何預測記錄"); return
    ...
    with tab2:
        ...
        with st.form("register_form"):
            ...
            with st.expander("📜 服務條款（請仔細閱讀）"):
                st.markdown("""
                **SHTSN 賽馬預測系統 服務條款**
                **1. 服務說明** 本系統提供賽馬預測數據及分析，僅供參考及娛樂用途，並非投注建議。用戶應自行判斷，所有投注決定及後果由用戶自行承擔。
                **2. 用戶責任** - 用戶必須年滿 18 歲。 - 用戶需確保所提供嘅資料真實、準確、完整。 - 用戶不得將本系統用於任何非法或不當用途。
                **3. 免責聲明** - 預測結果僅為演算法分析，不構成任何形式嘅投資建議或保證。 - 本系統不保證預測準確度，亦不對用戶因使用本系統而產生嘅任何損失負責。 - 用戶明白賽馬活動存在風險，應量力而為。
                **4. 付款與退款** - 用戶付款後即表示同意購買所選方案。 - 付款後不設退款，除非系統因技術問題未能提供服務。 - 管理員保留最終審核及拒絕退款嘅權利。
                **5. 帳戶安全** - 用戶需自行保管帳號及密碼，任何經由帳戶進行嘅活動均視為用戶本人所為。 - 如發現帳戶被盜用，應立即通知管理員。
                **6. 終止服務** - 管理員保留隨時終止或暫停用戶帳戶嘅權利，如用戶違反條款或進行不當行為。 - 終止後，用戶將無法使用系統服務，已付費用將不獲退還。
                **7. 條款修訂** 本系統有權隨時修訂服務條款，修訂後會於系統內公告。用戶繼續使用即表示同意最新條款。
                **8. 聯絡我們** 如有任何疑問，可透過 Telegram 聯絡管理員：@bryhjdjbrbxibvrjskofndhiebdpaq
            ...
            if submitted:
                if len(new_user)<3:
                    ...
                    st.error("❌ 密碼至少 4 個字")
                elif 'reg_verify_code' not in st.session_state or verify_code_input!=st.session_state['reg_verify_code'] or datetime.now()>st.session_state.get('reg_verify_expiry',datetime.now()):
                        ...
                        if CONFIG.get("enable_invite_reward",True) and invite_code_input:
                            for uid,u in users.items():
                                if u.get('invite_code')==invite_code_input:
                                    invited_by=uid; break
                            if not invited_by:
                                st.warning("⚠️ 邀請碼無效，請確認後再試。")
                        new_user_data={
                            'password':new_pass,'phone':phone,
...
def show_paywall():
    ...
    with st.form(key="payment_form"):
        ...
        if submitted:
            st.info("⏳ 正在處理你嘅申請...")
            if not plan_choice:
                st.error("❌ 請先選擇一個付費方案"); st.stop()
            if not st.session_state.get('logged_in',False):
                st.error("❌ 請先登入"); st.stop()
            original_price=get_plan_price(plan_choice)
            final_price=original_price
            discount_applied=False; discount_desc=""; promo_code_used=None
            if promo_input:
                try:
                    promos=load_promos()
                        ...
                        if expiry:
                            ...
                            if expiry_date>=datetime.now():
                                ...
                                if discount_type=='percentage':
                                    ...
                                    final_price=0; discount_desc="全免！"
            ...
            try:
                ...
                if save_payment_proofs(proofs):
                    ...
                    st.error("❌ 寫入付款記錄失敗，請檢查檔案權限"); st.stop()
            except Exception as e:
                st.error(f"❌ 提交過程中發生錯誤：{e}"); st.stop()
        ...
        if history:
            ...
            if user_records:
                ...
                if 'date' in df_rec:
                    ...
                    daily=df_rec.groupby(df_rec['date'].dt.date).agg(total=('is_hit','count'),hit=('is_hit',lambda x:(x==True).sum())).reset_index()
...
def admin_analytics():
    ...
    if users:
        ...
        if 'created_at' in df_users.columns:
            ...
            fig=px.line(daily,x='date',y=['new_users','cumulative'],title='每日新增用戶 & 累積用戶',labels={'value':'用戶數','date':'日期'})
            st.plotly_chart(fig,use_container_width=True)
        else:
...
def admin_promo_codes():
    ...
    with col1:
        ...
        if promos:
            ...
            if 'discount_type' not in df.columns: df['discount_type']='percentage'
            if 'discount_value' not in df.columns: df['discount_value']=0
            st.dataframe(df,use_container_width=True)
        else:
            st.info("暫無優惠碼")
    with col2:
        ...
        discount_type=st.selectbox("折扣類型",["percentage","fixed","free"],key="promo_discount_type",format_func=lambda x:{"percentage":"百分比（%折扣）","fixed":"固定金額（減$）","free":"完全免費"}.get(x,x))
        discount_value=st.number_input("折扣數值",min_value=0,value=20,key="promo_discount_value",help="百分比：20 = 8折（減20%）；固定金額：減指定金額；免費：無效")
        if st.button("產生優惠碼",key="gen_promo"):
            code=generate_promo_code()
            expiry=(datetime.now()+timedelta(days=duration)).isoformat()
            promos[code]={"used":False,"expiry":expiry,"created_at":datetime.now().isoformat(),"discount_type":discount_type,"discount_value":discount_value}
            save_promos(promos)
        ...
        if st.button("套用",key="apply_promo"):
            if code_input not in promos:
                ...
                if username_input not in users:
                    ...
                    save_users(users); save_promos(promos)
...
def admin_accuracy_monitor():
    ...
    try:
        ...
        if 'race_date' not in results_df.columns or 'race_no' not in results_df.columns or '馬名' not in results_df.columns or 'finish_position' not in results_df.columns:
            if '日期' in results_df.columns: results_df.rename(columns={'日期':'race_date'},inplace=True)
            if '場次' in results_df.columns: results_df.rename(columns={'場次':'race_no'},inplace=True)
            if '馬名' not in results_df.columns and 'horse_name' in results_df.columns: results_df.rename(columns={'horse_name':'馬名'},inplace=True)
            if 'finish_position' not in results_df.columns and '名次' in results_df.columns: results_df.rename(columns={'名次':'finish_position'},inplace=True)
        if 'race_date' in results_df.columns and 'race_no' in results_df.columns and '馬名' in results_df.columns and 'finish_position' in results_df.columns:
            ...
            for rec in records:
                ...
                date_str=rec['date']; race_no=rec['race']; horse=rec['horse']
                matched=results_df[(results_df['race_date'].dt.strftime('%Y-%m-%d')==date_str)&(results_df['race_no']==race_no)&(results_df['馬名']==horse)]
    ...
    if 'date' in df_records:
        ...
        daily=df_records.groupby(df_records['date'].dt.date).agg(total=('is_hit','count'),hit=('is_hit',lambda x:(x==True).sum())).reset_index()
        daily['hit_rate']=daily['hit']/daily['total']
...
def admin_subscription():
    ...
    if not paid_users:
        ...
        for col in required_cols:
            if col not in df_paid.columns: df_paid[col]=None
        ...
        for uid,u in users.items():
            if u.get('group')=='VIP' and u.get('expiry_date'):
                try:
                    ...
                    if exp<today:
                        u['group']='free'; u['is_paid']=False; u['predictions_limit']=CONFIG["free_limit"]; u['plan']=None
...
def admin_content():
        ...
        if st.button("📤 發佈公告",type="primary",key="publish_ann"):
            if not title or not content_text:
                ...
                new_ann={
                    ...
                    "title":title,"content":content_text,"type":ann_type,
                    "target":target_group,
    ...
    for ann in announcements:
        if ann.get('status')=='active' and ann.get('end_date'):
            ...
            if end<today: ann['status']='expired'
    save_json(CONTENT_FILE,content)
    content=load_json(CONTENT_FILE)
...
def admin_payment_review():
    ...
    with st.expander("🔍 篩選與搜尋",expanded=True):
        ...
        with col_s2:
            status_filter=st.selectbox("狀態篩選",["全部","pending","approved","rejected"],index=1,format_func=lambda x:{"pending":"待審核","approved":"已批准","rejected":"已拒絕","全部":"全部"}.get(x,x))
        with col_s3:
            ...
            with cols[4]:
                if status=="pending":
                    if st.button("✅ 批准",key=f"approve_{original_idx}"):
                        _approve_payment(rec,proofs_data); st.rerun()
                    if st.button("❌ 拒絕",key=f"reject_{original_idx}"):
                        _reject_payment(rec,proofs_data); st.rerun()
                elif status=="approved":
                    if st.button("↩️ 退款",key=f"refund_{original_idx}"):
                        _refund_payment(rec,proofs_data); st.rerun()
                else:
                    st.write("已處理")
            st.divider()
...
def _approve_payment(rec,proofs_data):
    try:
        ...
        if not username:
            st.error("❌ 記錄中缺少 username"); return
        users=load_users()
        if username not in users:
            st.error(f"❌ 用戶 {username} 不存在"); return
...
def _refund_payment(rec,proofs_data):
    try:
        ...
        if save_users(users):
            ...
            st.error("❌ 儲存 users.json 失敗"); return
        rec['status']='rejected'
        rec['refunded']=True
...
def main():
    ...
    active_anns=[]
    for ann in announcements:
        if ann.get('status')!='active': continue
        ...
        if target!='全部用戶':
            if not st.session_state.get('logged_in',False): continue
            user=load_users().get(st.session_state.username,{})
            group=user.get('group','free')
            if target=='付費用戶' and group not in ['paid','VIP','super_admin']: continue
            if target=='VIP' and group not in ['VIP','super_admin']: continue
            if target=='免費用戶' and group!='free': continue
