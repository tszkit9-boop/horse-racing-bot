...
def main():
    ...
    try:
        ...
        with col_date:
            selected_date = st.date_input("📅 選擇日期", value=default_date, key="predict_date_mid")
            date_str = selected_date.strftime("%Y-%m-%d")
        with col_race:
            race_no = st.selectbox("🏇 選擇場次", list(range(1, max_race + 1)), index=0, key="predict_race_mid")
        with col_btn:
            predict_btn = st.button("🚀 執行預測", type="primary", use_container_width=True, key="predict_btn_mid_unique")
