...
EXPECTED_FEATURES = [
    'horse_age', 'horse_rating', 'weight', 'draw', 'jockey_win_rate',
    'trainer_win_rate', 'avg_speed', 'last_race_rank', 'last_race_distance',
    'career_win_rate', 'career_place_rate', 'track_win_rate',
    'distance_win_rate', 'going_win_rate', 'recent_form', 'form_trend',
    'weight_adjustment', 'gear_change', 'barrier_trial_rank',
    'days_since_last_race', 'horse_condition', 'trainer_track_win_rate',
    'jockey_track_win_rate', 'race_class', 'handicap_weight',
    'season_win_rate', 'month_win_rate', 'race_course_win_rate',
    'horse_age_factor', 'draw_factor', 'weight_factor', 'jockey_horse_win_rate',
    'trainer_horse_win_rate', 'recent_avg_rank', 'recent_win_rate',
    'horse_consistency', 'value_index'
]
...
def save_users(data):
    return save_json('users.json', data)

def load_accuracy():
    return load_json('accuracy.json', {'records': []})

def save_accuracy(data):
    return save_json('accuracy.json', data)

def load_finance():
    return load_json('finance.json', {'total_income': 0, 'monthly_income': 0, 'yearly_income': 0})

def save_finance(data):
    return save_json('finance.json', data)
...
def load_system_config():
    ...
    return load_json('system_config.json', default)

def save_system_config(data):
    return save_json('system_config.json', data)

def load_promos():
    return load_json('promo_codes.json', {})

def save_promos(data):
    return save_json('promo_codes.json', data)

def load_logs():
    return load_json('admin_log.json', {'logs': []})

def save_logs(data):
    return save_json('admin_log.json', data)
...
def record_prediction(username, date_str, race_no, horse_name, probability):
    """記錄用戶預測"""
    acc = load_accuracy()
...
def show_paywall():
    ...
    <div style="background:linear-gradient(135deg,#fff3e0,#ffe0b2);border-radius:16px;padding:30px 20px;text-align:center;border:2px dashed #ff6f00;margin:10px 0;">
        ...
        <h3 style="color:#e65100;margin:10px 0;">免費預測次數已用完</h3>
        <p style="color:#bf360c;font-size:16px;">升級付費方案即可繼續使用預測服務</p>
        <div style="margin-top:15px;">
            <span style="background:#ff6f00;color:white;padding:10px 30px;border-radius:20px;font-weight:bold;font-size:16px;cursor:pointer;">💎 立即升級</span>
        </div>
    </div>
...
def get_all_pending_requests():
    """獲取所有待審核的付款請求"""
    payment_proofs = load_json('payment_proofs.json', {'proof_records': []})
    pending = []
    
    for record in payment_proofs.get('proof_records', []):
...
def approve_payment_request(username, request_id, admin_name):
    """批准付款請求"""
    payment_proofs = load_json('payment_proofs.json', {'proof_records': []})
    
    for record in payment_proofs.get('proof_records', []):
        if record.get('id') == request_id and record.get('status') == 'pending':
            record['status'] = 'approved'
            record['approved_by'] = admin_name
            record['approved_at'] = datetime.now().isoformat()
            
            users = load_users()
            if username in users:
                users[username]['is_paid'] = True
                users[username]['group'] = 'paid'
                users[username]['predictions_limit'] = -1
                users[username]['paid_date'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                save_users(users)
            
            save_json('payment_proofs.json', payment_proofs)
            return True, f"✅ 已批准 {username} 的付款申請"
    
    return False, "❌ 找不到該申請"

def reject_payment_request(username, request_id, admin_name):
    """拒絕付款請求"""
    payment_proofs = load_json('payment_proofs.json', {'proof_records': []})
    ...
    for record in payment_proofs.get('proof_records', []):
        if record.get('id') == request_id and record.get('status') == 'pending':
            ...
            record['rejected_by'] = admin_name
            record['rejected_at'] = datetime.now().isoformat()
            
            save_json('payment_proofs.json', payment_proofs)
            return True, f"❌ 已拒絕 {username} 的付款申請"
    
    return False, "❌ 找不到該申請"

# ============================================================
# ⭐ 核心修復：run_prediction 函數（完整修復版）
# ============================================================
...
def run_prediction(date_str, race_no):
    ...
    try:
        ...
        mask = (df['race_date'].dt.strftime('%Y-%m-%d') == date_str) & (df['race_no'] == race_no)
        race_data = df[mask].copy()
        
        if race_data.empty:
            return None, "沒有找到該場次數據"
        ...
        if len(available_cols) < 10:
            return None, f"可用特徵不足：{len(available_cols)}/36"
        
        X = race_data[available_cols].copy()
        ...
        try:
            xgb = XGBClassifier()
            xgb.load_model('hk_racing_model.pkl')
            models.append(xgb)
            weights.append(CONFIG.get('xgb_weight', 25))
        except:
            pass
        ...
        try:
            ...
            cat.load_model('hk_catboost_model.cbm')
            models.append(cat)
            weights.append(CONFIG.get('cat_weight', 1))
        except:
            pass
        
        if not models:
            return None, "無法載入任何模型"
        ...
        for model in models:
            try:
                ...
                probs.append(np.full(len(X), 0.1))
        
        # 加權平均
        weights = np.array(weights) / sum(weights)
        final_probs = np.zeros(len(X))
        for i, prob in enumerate(probs):
            final_probs += weights[i] * prob
        
        # 構建結果 DataFrame
        result = race_data[['horse_name', 'draw']].copy()
        result['預測勝率'] = final_probs
        result['馬匹名稱'] = result['horse_name']
        result['檔位'] = result['draw']
        result = result.sort_values('預測勝率', ascending=False)
        
        # ============================================================
        # ⭐ 修復部分：安全處理檔位值
        # ============================================================
        
        def safe_draw_compare(draw_value):
            """安全地比較檔位值"""
            try:
                # 檢查是否為 None 或 NaN
                if draw_value is None or pd.isna(draw_value):
                    return None
                
                # 如果是數字，直接轉為整數
                if isinstance(draw_value, (int, float)):
                    return int(draw_value)
                
                # 如果是字串，嘗試提取數字
                if isinstance(draw_value, str):
                    # 嘗試匹配數字（例如 "1", "1(4)", "1 檔"）
                    match = re.search(r'(\d+)', draw_value)
                    if match:
        ...
        # 安全計算值博指數
        def safe_calculate_value_index(row):
            prob = row.get('預測勝率', 0)
            draw = row.get('檔位', None)
            
            # 基礎指數
            index = prob * 100
            
            # 安全轉換檔位
            draw_num = safe_draw_compare(draw)
            
            # 檔位加權（僅當 draw_num 有效時）
        ...
        # 計算信心指數
        def safe_calculate_confidence(row):
            prob = row.get('預測勝率', 0)
            draw = row.get('檔位', None)
        ...
        # 應用計算
        result['信心指數'] = result.apply(safe_calculate_confidence, axis=1)
        result['值博指數'] = result.apply(safe_calculate_value_index, axis=1)
        
        # 生成彩池推薦文字
        pool_text = generate_pool_recommendations(result)
        
        return result, pool_text
        
    except Exception as e:
        return None, f"預測失敗：{str(e)}"

def generate_pool_recommendations(result):
    """生成彩池推薦文字"""
    ...
    lines = []
    lines.append(f"🏆 獨贏推薦：{top1['馬匹名稱']}（勝率 {top1['預測勝率']:.2%}）")
    
    if len(top4) >= 2:
        lines.append(f"🔗 連贏推薦：{top4.iloc[0]['馬匹名稱']} + {top4.iloc[1]['馬匹名稱']}")
    
    if len(top4) >= 3:
        names = '、'.join(top4.iloc[:3]['馬匹名稱'].tolist())
        lines.append(f"🥉 三重彩推薦：{names}（順序）")
    
    if len(top4) >= 4:
        names = '、'.join(top4.iloc[:4]['馬匹名稱'].tolist())
        lines.append(f"🏅 四重彩推薦：{names}（順序）")
    
    confidence = top1.get('信心指數', 50)
    if confidence >= 70:
        rating = "🟢 高信心"
    elif confidence >= 40:
        rating = "🟡 中等信心"
    ...
    lines.append(f"📊 信心評級：{rating}（{confidence}%）")
...
def show_user_dashboard(username):
    ...
    st.subheader(f"👤 歡迎，{username}！")
    
    col1, col2, col3 = st.columns(3)
    group = user_data.get('group', 'free')
    col1.metric("🏷️ 用戶級別", group.upper())
    
    limit = user_data.get('predictions_limit', CONFIG.get('free_limit', 2))
    if limit == -1:
        col2.metric("📊 剩餘場次", "♾️ 無限")
    else:
        used = user_data.get('free_usage', 0)
        remain = max(0, limit - used)
        col2.metric("📊 剩餘場次", remain)
    
    total_usage = user_data.get('total_usage', 0)
    col3.metric("📊 總預測次數", total_usage)
    
    if group == 'VIP' and user_data.get('expiry_date'):
        try:
            exp = pd.to_datetime(user_data['expiry_date'])
            days_left = (exp - datetime.now()).days
            if days_left > 0:
                st.info(f"💎 VIP 有效期至：{exp.strftime('%Y-%m-%d')}（剩餘 {days_left} 天）")
            else:
                st.warning("⚠️ VIP 已過期，請續期")
        except:
            pass
