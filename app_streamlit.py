...
# -*- coding: utf-8 -*-
"""
賽馬預測系統 - Streamlit 網頁版（完整修正版）
處理：缺賠率、場次格式 'Race 1'、完整 36 特徵
"""
...
try:
    ...
    if 'race_date' not in df_sched.columns and '比賽日期' in df_sched.columns:
        df_sched.rename(columns={'比賽日期': 'race_date'}, inplace=True)
    if 'race_date' in df_sched.columns:
        df_sched['race_date'] = df_sched['race_date'].astype(str).str.replace('/', '-')
