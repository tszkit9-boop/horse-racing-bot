...
def admin_lottery_config():
    ...
    if all_records:
        ...
        st.download_button("📥 下載全部記錄", data=csv,
                          file_name=f"lottery_records_{datetime.now().strftime('%Y%m%d')}.csv",
                          mime="text/csv", key="download_lottery_records")
    else:
        st.info("暫無抽獎記錄")
