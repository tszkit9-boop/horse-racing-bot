...
with st.expander("Q2：呢個系統準唔準？"):
    st.write("我哋會每日公開更新命中率，你可以喺「後台 -> 預測監控」睇到實際表現。賽馬本身有隨機性，命中率會隨時間同數據累積而變化。")

with st.expander("Q3：免費定收費？"):
    st.write("有免費預測次數（由 free_limit 控制）。如果你想支持我哋，可以透過「❤️ 打賞支持」解鎖 VIP 天數，打賞屬自願性質。")
...
with st.expander("Q10：有咩會員級別？"):
    st.write("free（免費）、paid（一般付費）、VIP（付費會員）、super_admin（管理員）。")

with st.expander("Q11：free 同 VIP 有咩分別？"):
    st.write("free 有限預測次數，只可睇獨贏、位置、連贏、位置Q；VIP 可睇全部彩池（包括三重彩、單T、四重彩、二重彩、四連環、孖寶、三寶、六環彩）。")

with st.expander("Q12：點樣打賞支持？"):
    st.write("去「❤️ 打賞支持」，揀一個打賞金額，然後用 FPS 轉數快過數，再撳「我已經付款」提交。管理員審核後會自動加 VIP 天數。")

with st.expander("Q13：打賞要等幾耐先加 VIP？"):
    st.write("通常即日處理，視乎管理員審核時間。批准後會自動加到你嘅帳戶。")

with st.expander("Q14：打賞可以退款嗎？"):
    st.write("打賞屬自願性質，一般不設退款。")

with st.expander("Q15：打賞金額有咩選擇？"):
    st.write("由管理員喺後台設定，通常有幾個選項（例如 $20 加 3 日、$50 加 7 日、$100 加 30 日、$300 加 90 日）。實際選項請以網站顯示為準。")

# ========== 四、彩池相關 ==========
st.header("四、彩池相關")

with st.expander("Q16：有咩彩池可以睇？"):
    st.write("單場有 9 種：獨贏、位置、連贏、位置Q、三重彩、單T、四重彩、二重彩、四連環；跨場有孖寶、三寶、六環彩。")

with st.expander("Q17：點解有啲彩池睇唔到？"):
    st.write("彩池按會員級別開放，升級會員或者打賞支持就可以解鎖。")

with st.expander("Q18：跨場彩池點用？"):
    st.write("先執行「一鍵預測所有場次」，再手動揀起始場次，就會顯示孖寶、三寶、六環彩。")

with st.expander("Q19：每個彩池會出幾多個組合？"):
    st.write("每個彩池只顯示一個推薦組合。")

# ========== 五、抽獎 / 商城 ==========
st.header("五、抽獎 / 商城")

with st.expander("Q20：抽獎幾時重置？"):
    st.write("每晚香港時間 00:00 自動重置，每日派發 1 次抽獎機會。")

with st.expander("Q21：抽獎有咩獎品？"):
    st.write("虛擬幣、VIP 天數、免費預測次數、優惠碼、自訂獎品、或者「唔中」。")

with st.expander("Q22：商城有咩買？"):
    st.write("預測次數、VIP 天數、抽獎次數、稱號、神秘盒等。")

with st.expander("Q23：商城物品可以改價錢嗎？"):
    st.write("管理員可以喺後台直接編輯價錢、庫存同名稱。")

# ========== 六、帳戶 / 技術 ==========
st.header("六、帳戶 / 技術")

with st.expander("Q24：點樣改密碼？"):
    st.write("登入後入「個人中心」→「更改密碼」。")

with st.expander("Q25：唔記得密碼點算？"):
    st.write("請聯絡管理員協助重置。")

with st.expander("Q26：點樣睇自己嘅預測記錄？"):
    st.write("登入後入「個人中心」→「預測記錄」。")

with st.expander("Q27：點解有時網站好慢？"):
    st.write("因為我哋用咗雲端數據庫（Supabase），每次操作都要經互聯網讀寫。通常幾秒內完成，如果持續緩慢，請稍後再試。")

with st.expander("Q28：點解預測記錄突然冇咗？"):
    st.write("我哋已經改用 Supabase 雲端數據庫，理論上數據唔會再因為網站重啟而消失。如果發現記錄唔見咗，請聯絡管理員。")

# ========== 七、邀請 / 獎勵 ==========
st.header("七、邀請 / 獎勵")

with st.expander("Q29：點樣邀請朋友？"):
    st.write("喺「個人中心」攞你嘅邀請碼，分享俾朋友註冊時填寫。")

with st.expander("Q30：邀請有咩獎勵？"):
    st.write("多級下線獎勵：level1 +5 次、level2 +2 次、level3 +1 次。")

with st.expander("Q31：點樣升級等級？"):
    st.write("透過預測、邀請、消費等累積，等級由銅牌 → 銀牌 → 金牌 → 鑽石 → 傳說。")

# ========== 八、法律 / 免責 ==========
st.header("八、法律 / 免責")

with st.expander("Q32：呢個系統係咪賭博？"):
    st.write("SHTSN 係數據分析同預測工具，唔涉及直接投注。用戶需自行決定是否投注，並自負風險。")

with st.expander("Q33：預測結果有冇保證？"):
    st.write("冇。賽馬有隨機性，預測僅供參考，唔構成任何投注建議。")

with st.expander("Q34：資料會唔會外洩？"):
